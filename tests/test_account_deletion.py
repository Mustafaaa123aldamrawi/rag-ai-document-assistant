...
def test_supabase_account_deletion_uses_legacy_service_role_bearer(monkeypatch):
    calls = []
    ...
    monkeypatch.setenv("AVIA_SUPABASE_SERVICE_ROLE_KEY", "legacy-service-role-jwt")
    ...
    assert calls[0][1]["Authorization"] == "Bearer legacy-service-role-jwt"
    assert calls[0][1]["apikey"] == "legacy-service-role-jwt"


def test_supabase_account_deletion_uses_new_secret_key_as_apikey_only(monkeypatch):
    calls = []
    ...
    monkeypatch.setenv("AVIA_ACCOUNT_DELETE_PROVIDER", "supabase")
    monkeypatch.setenv("AVIA_SUPABASE_URL", "https://project.supabase.co")
    monkeypatch.setenv("AVIA_SUPABASE_SERVICE_ROLE_KEY", "sb_secret_example")
    monkeypatch.setattr(account_deletion.requests, "delete", fake_delete)

    result = account_deletion.delete_auth_identity("user-123")

    assert result["identity_deleted"] is True
    assert calls[0][1]["apikey"] == "sb_secret_example"
