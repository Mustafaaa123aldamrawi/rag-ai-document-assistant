...
def build_document_overview_instruction(question, query_route):
    if (
        ...
        or not is_document_overview_question(question)
    ):
        return ""
...
def extract_primary_drawing_number(source_drawing_text):
...
def render_pdf_pages_for_vision(pdf_file, zoom=2.0, max_pages=None, start_page=1):
    """
            ...
            if rendered_drawing_pages:
                ...
                try:
                    ...
                    cleanup_prompt = (
                        ...
                        "When two region analyses give conflicting model numbers, display sizes, quantities, "
                        "or technical values, do not silently choose one. "
                        "Keep the value only if it is consistently supported by the region analyses; "
                        "otherwise mark the field as unclear and list the conflicting readings briefly. "
                        "Never change an equipment size, model number, or quantity during cleanup. "
                        "Do not merge separate installation-note lines into a single sentence. "
                    ...
                    structured_prompt = (
                        ...
                        "If equipment text conflicts across region analyses, use null for the uncertain model or value rather than selecting one arbitrarily. "
                        "Do not classify labels such as FB, HIDDEN D1, dimensions, symbols, or other ambiguous callouts as equipment unless they are clearly identified as AV devices. "
                        "Room schedulers, codecs, cameras, displays, mounts, power devices, and control devices belong under equipment when clearly supported. "
                        "Room schedulers must be classified as equipment, not installation_notes. "
                        "Do not alter, normalize, autocorrect, or rewrite alphanumeric model numbers. "
                        "If different source analyses contain conflicting model-number readings, set model to null and describe the conflicting readings in uncertainty_notes. "
                        "drawing_title must always be a single string, never a list. "
                        "If multiple drawing titles are clearly supported, join them into one string separated by ' / '. "
                        "Do not invent sheet labels such as Sheet 01, Sheet 02, or Sheet 03 unless those exact labels are explicitly supported by the source analyses. "
                        "Add an uncertainty_notes list for unresolved conflicting readings. "
                        "Set confidence only to one of these values: \"high\", \"medium\", or \"low\". "
                        "Use \"high\" when the same equipment detail is clearly supported by multiple region analyses or is clearly readable in one region without conflict. "
                        "Use \"medium\" when the equipment is clearly identified but some detail such as model or quantity is incomplete. "
                        "Use \"low\" when the equipment reading is ambiguous, partially readable, or conflicts across regions. "
                        "If a device is clearly identifiable as AV equipment but its manufacturer, model, or quantity is uncertain, keep the device in equipment and set only the uncertain fields to null. "
                        "Exclude an item from equipment only when the identity of the device itself is genuinely unreadable or ambiguous. "
                        "Do not place ambiguous symbols, isolated labels, unexplained dimensions, or uncertain OCR fragments in installation_notes. "
                        "Move items such as FB, HIDDEN D1, N, AV, isolated numbers, or unclear dimensional text to uncertainty_notes unless their meaning is explicitly clear from the drawing. "
                        "Installation notes must contain only clearly readable installation instructions, weights, mounting requirements, conduit requirements, scale information, or similarly explicit technical notes. "
                        "If a technical value looks suspicious or conflicts with other readings, place it in uncertainty_notes instead of treating it as confirmed. "
                        "Do not classify structural backing, mounting instructions, safety-factor notes, conduit notes, pull-string notes, dimensions, view labels, or field-coordinate labels as equipment. "
                        "Do not classify architectural labels such as HIDDEN DOOR, doors, walls, cabinets, or similar construction annotations as room_areas unless they clearly name an actual room or functional area. "
                        "Structural backing requirements belong under installation_notes, not equipment. "
                    ...
                    try:
                        ...
                        deduplicated_references = deduplicate_references(
                            references
                        )
                        ...
                        for equipment_item in structured_drawing_data.get("equipment", []):
                            ...
                            if equipment_name == "AM-3100-WF AIR MEDIA":
                                ...
                                if isinstance(model, str) and model.strip() in {
                                    "3100-WF",
                                    "AM-3100-WF AIR MEDIA",
                                }:
...
"""
        ...
        """.strip()
        document_overview_instruction = build_document_overview_instruction(
            question,
            query_route,
        )
        prompt = f"""
